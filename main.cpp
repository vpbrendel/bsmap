...
int check_ifile_format(string &filename, int gz_flag) {
	...
	if(gz_flag) {
		...
		if(sam_open(filename.c_str(), "rb")!=0) {message<<" \t(format: BAM)\n"; info(1); return 3;} //BAM         
        if(sam_open(filename.c_str(), "r")!=0) {message<<" \t(format: SAM)\n"; info(1); return 2;} //SAM
	}	 
	else {
...
void RunProcess(void) {
	...
	if(param.pairend) {
		...
		if(param.input_format==3) {
		    bam_destroy1(read_a.SAM_b); sam_close(read_a.SAM_fp);
		    bam_destroy1(read_b.SAM_b); sam_close(read_b.SAM_fp);
        }
