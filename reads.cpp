...
void ReadClass::InitIndex(ifstream &fin, igzstream &gzfin, string filename) {
    ...
    case 2: //sam
	case 3: //bam
		SAM_fp=sam_open(filename.c_str(), "rb");
		SAM_h=sam_hdr_read(SAM_fp);
		SAM_b=bam_init1();
		maxi=(param.read_start-1)*(1+param.pairend);
		for(i=0;i<maxi;i++) if(sam_read1(SAM_fp,SAM_h,SAM_b)<0) break;
	}
...
int ReadClass::LoadBatchReads(ifstream &fin, igzstream &gzfin, int readset)  // readset {0: single-end, 1:pair-end set1, 2:pair-end set2)  
{
	...
	if (param.input_format<2) //.fa and .fq format
		...
		//cout<< "index:"<<_index<<endl;
            ...
            if(readset==2) if (sam_read1(SAM_fp,SAM_h,SAM_b)<0) break;
    		if (sam_read1(SAM_fp,SAM_h,SAM_b)<0) break;
      		p->index=_index;
       		p->name=string((char*)bam_get_qname(SAM_b));
           //  
       		l_seq=min((bit32_t)SAM_b->core.l_qseq,(bit32_t)param.max_readlen);
       		p->seq.assign(l_seq,0); p->qual.assign(l_seq,0);
       		s=(char*) bam_get_seq(SAM_b);  t=(char*) bam_get_qual(SAM_b);
            if(readset) {
       		...
       		for(i=0;i<l_seq;i++){
       		    p->seq[i]=seq_nt16_table[bam_seqi(s,i)];
       		    p->qual[i]=t[i]+33;
       		}
			//cout<<p->name<<" "<<p->seq<<endl;
            if(readset==1) if (sam_read1(SAM_fp,SAM_h,SAM_b)<0) break;
		}
